...
var Logger = (function () {
    ...
    Logger.prototype.doMongo = function () {
                ...
                collection.findOne({ id: id }, function (err, doc) {
                    ...
                    if (doc != undefined) {
                        if (doc.names.indexOf(obj.nm) === -1) {
                            ...
                            collection.save(doc, function (err2, result) {
                                if (err2) {
                                    log(err2.toString());
                                }
                            });
                        ...
                        collection.update({ id: id }, { id: id, names: [obj.nm] }, { w: 1, upsert: true }, function (err3, result) {
                            if (err3) {
                                log(err3.toString());
                            }
                        });
        ...
        MongoClient.connect("mongodb://127.0.0.1:27017/Incoming", function (err, db) {
            ...
            database = db;
            db.collection("IDDB", function (err2, col) {
                if (err2) {
                    throw err2;
                }
